...
__input(void)
{
 string = (unsigned char *)calloc(10, sizeof(char));
...
__preproces(void)
{
 ...
 for (; string[ind]; ind++) {
  ...
  if ((string[ind] == ' ' || string[ind] == '\t')) {
   ...
   return 1;
  } else if (((string[ind] >= 'a' && string[ind] <= 'z') || (string[ind] >= 'A' && string[ind] <= 'Z') || string[ind] == '_')) {
...
__is_token(unsigned ind, unsigned ind_token)
{
 ...
 if ((symbol >= '0' && symbol <= '9') || (symbol == '-' && ((!ind_token || (arr_tokens[ind_token - 1].type == SIGN || (arr_tokens[ind_token - 1].type == OPENING_BLACKET))) && (string[ind + 1] >= '0' && string[ind + 1] <= '9')))) {
  unsigned char t_symbol = 0;
  unsigned ind_end = ind + 1;
  char int_or_float = 1;
  for (; (string[ind_end] >= '0' && string[ind_end] <= '9') || string[ind_end] == '.'; ind_end++, offset++) {
   if (string[ind_end] == '.') {
  ...
  unsigned ind_end = ind + 1;
  for (; ((string[ind_end] >= 'a' && string[ind_end] <= 'z') || (string[ind_end] >= 'A' && string[ind_end] <= 'Z') || string[ind_end] == '_'); ind_end++, offset++);
...
{
 unsigned char list_name_funcs[] = "log\0arcsin\0sin\0cos\0\0";
 unsigned char list_name_constant[] = "pi\0x\0X\0\0";
 unsigned char *point_list = 
# 243 "get_tokens.c" 3 4
                            ((void *)0)
# 243 "get_tokens.c"
                                ;
